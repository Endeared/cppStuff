...
struct Deck{ // makes the deck for all 52 cards with all 4 suits & 13 numbers
  ...
  void warGame(int Player1[], int Player2[], int thisDeck[]){ // now initializes the deck that was passed in
    shuffleCards();
    dealCards(Player1, Player2);
    int P1Score = 0;
    int P2Score = 0;
    for (int match = 0; match < 26; match++){
      int P1Card = thisDeck[Player1[match]]; // changed so it checks for the value from the deck at the players index
      int P2Card = thisDeck[Player2[match]]; // changed so it checks for the value from the deck at the players index
      if (P1Card > P2Card){
        ...
        cout << "It's a tie!" << endl; // forgot to add endl here
      }
    }//end of the for loop
...
int main() {
  ...
  int Player1[26], Player2[26], thisDeck[52]; // makes the 2 arrays for the 2 players and array for thisDeck
  ...
  cout << endl << endl;
  for (int i = 0; i < 52; i++){ // iterates through all values in my_deck.arrCards and passes them into thisDeck
      thisDeck[i] = my_deck.arrCards[i].value;
  }
  
  my_deck.warGame(Player1, Player2, thisDeck); // passes thisDeck into wargame func
}